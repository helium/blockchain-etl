-module(be_txn).
-export([to_type/1, to_json/1, to_actors/1]).
...
to_actors(blockchain_txn_poc_receipts_v1, T) ->
    ToActors = fun(undefined, Acc0) ->
                   ...
                   (Elem, {ChallengeeAcc0, WitnessAcc0}) ->
                       ChallengeeAcc =
                           [{"challengee",
                             ?BIN_TO_B58(blockchain_poc_path_element_v1:challengee(Elem))} | ChallengeeAcc0],
                       WitnessAcc = lists:foldl(fun(W, WAcc) ->
                                                        [{"witness",
                                                          ?BIN_TO_B58(blockchain_poc_witness_v1:gateway(W))} | WAcc]
...
to_actors(blockchain_txn_rewards_v1, T) ->
    ToActors = fun(R, {PayeeAcc0, GatewayAcc0}) ->
                       ...
                       GatewayAcc = case blockchain_txn_reward_v1:gateway(R) of
                                        ...
                                        G -> [{"gateway", ?BIN_TO_B58(G)} | GatewayAcc0]
                                    end,
                       {PayeeAcc, GatewayAcc}
               end,
