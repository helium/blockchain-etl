...
%% api
-export([block_height/1, write_block/3]).
...
load_block(Conn, Hash, Block, _Sync, Ledger, State = #state{}) ->
    ...
    case os:getenv("BLOCKS_DIR") of
        false ->
            ok;
        Dir ->
            case write_block(BlockHeight, Dir, blockchain_worker:blockchain()) of
                ok ->
                    ok;
                {error, Error} ->
                    lager:warning("Failed to write block ~p: ~p", [BlockHeight, Error])
            end
    end,
...
-spec write_block(pos_integer(), filename:dir(), blockchain:blockchain()) -> ok | {error, term()}.
write_block(Height, Path, Chain) ->
    case blockchain:get_raw_block(Height, Chain) of
        not_found ->
            {error, not_found};
        {ok, BinBlock} ->
            Filename = filename:join(Path, integer_to_list(Height)),
            file:write_file(Filename, BinBlock);
        {error, _} = Error ->
            Error
    end.
