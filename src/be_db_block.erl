...
maybe_write_snapshot(Height, SnapshotHash, SnapshotDir, Chain) ->
    {ok, BinSnap} =
        case blockchain:get_snapshot(SnapshotHash, Chain) of
            {error, NF} when NF =:= not_found; NF =:= file_not_found ->
                blockchain:get_snapshot(Height, Chain);
            Other ->
                Other
        end,
    Filename = filename:join([SnapshotDir, io_lib:format("snap-~p", [Height])]),
    {ok, _} = blockchain:save_compressed_bin_snapshot(Filename, BinSnap),
    {ok, Sz, FHash} = blockchain:maybe_get_compressed_snapdata(Filename),
    LatestMap = #{
        height => Height,
        hash => base64url:encode(SnapshotHash),
        compressed_hash => base64url:encode(FHash),
        compressed_size => Sz
    },
